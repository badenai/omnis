...
export default function StatusPanel({ agent }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Stat row — 4 compact cards across full width */}
      ...
      {/* Lower section: channels left, actions + jobs right */}
      <div className="grid grid-cols-2 gap-4">
        {/* Channel Status */}
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Channel Status</h3>
          <div className="space-y-2">
            {channels.map((ch) => (
              <div key={ch.handle} className="flex items-center justify-between bg-gray-900 border border-gray-800 rounded-lg px-3 py-2.5">
                <div>
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded text-xs font-medium transition-colors shrink-0 ml-2"
                >
                  Collect Now
                </button>
              </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 rounded text-sm font-medium transition-colors text-left"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-violet-700 hover:bg-violet-600 disabled:opacity-50 rounded text-sm font-medium transition-colors text-left"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-emerald-700 hover:bg-emerald-600 disabled:opacity-50 rounded text-sm font-medium transition-colors text-left"
              >
          ...
          {agentJobs.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Scheduled Jobs</h3>
              <div className="space-y-1">
                {agentJobs.map((job) => (
                  <div key={job.id} className="flex justify-between text-xs bg-gray-900 border border-gray-800 rounded px-3 py-2">
                    <span className="text-gray-300 truncate mr-2">{job.name}</span>
      ...
      {discoveredSources?.content && (
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Discovered Sources</h3>
          <pre className="bg-gray-900 border border-gray-800 rounded-lg p-4 text-xs text-gray-300 whitespace-pre-wrap overflow-auto max-h-48">
            {discoveredSources.content}
          </pre>
        </div>
      )}
