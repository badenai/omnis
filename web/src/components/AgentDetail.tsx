...
function parseSuggestions(text: string): SuggestionCard[] {
  const parts = text.split(/\n(?=\d+\. )/);
  return parts.flatMap((part, i) => {
    const titleMatch = part.match(/\*\*(.*?)\*\*/);
    const reasoningMatch = part.match(/\*\*Reasoning:\*\*\s*([\s\S]*)/);
...
type ActiveTab = 'knowledge' | 'channels' | 'inbox' | 'session';
...
export default function AgentDetail() {
  ...
  const parsedSuggestions = useMemo(() => {
    ...
    return parseSuggestions(suggestionsData.suggestions);
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {viewMode === 'manage' && (
        <div style={{ flex: 1, minHeight: 0, position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, minHeight: 0, border: '1px solid var(--color-border-subtle)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                {activeTab === 'session' && (
                  <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                    ...
                    {/* Soul Evolution Suggestions — hero at top */}
                    <div style={{ padding: '16px 20px 20px', borderBottom: '1px solid var(--color-border-subtle)', flexShrink: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                        <div style={{ ...monoLabel, fontSize: 11 }}>
                          <svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: 'var(--color-accent)' }}>
                        ...
                        parsedSuggestions.length > 0 ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            {parsedSuggestions.map(s => (
                              <div key={s.number} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                                <div style={{ width: 22, height: 22, borderRadius: '50%', backgroundColor: 'var(--color-accent-glow)', border: '1px solid var(--color-accent-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 700, color: 'var(--color-accent)' }}>{s.number}</span>
                                </div>
                                <div>
                                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)', marginBottom: 3 }}>{s.title}</div>
                                  {s.reasoning && (
                                    <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>{s.reasoning}</div>
                                  )}
                                </div>
                              </div>
                            ))}
                          ...
                          <pre style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--color-text-secondary)', whiteSpace: 'pre-wrap', lineHeight: 1.7, margin: 0 }}>
                            {suggestionsData.suggestions}
                          </pre>
                        )
                      ) : (
                        <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>
                          {isFetchingSuggestions ? 'Generating...' : 'No suggestions yet. Run a consolidation to generate them.'}
                        </div>
                      )}
                    </div>
          ...
          {/* Right: Inspector */}
          <aside style={{ width: 360, flexShrink: 0, borderLeft: '1px solid var(--color-border-subtle)', display: 'flex', flexDirection: 'column', overflowY: 'auto', backgroundColor: 'var(--color-surface-1)' }}>
            <div style={{ position: 'sticky', top: 0, padding: '12px 20px', borderBottom: '1px solid var(--color-border-subtle)', display: 'flex', alignItems: 'center', gap: 6, backgroundColor: 'rgba(18,18,22,0.96)', backdropFilter: 'blur(8px)', zIndex: 10 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)' }}>Agent Inspector</span>
            </div>
      ...
      {viewMode === 'manage' && (
        <>
          <button
            ...
            style={{ position: 'fixed', bottom: 32, right: 32 + 360 + 12, zIndex: 50, width: 48, height: 48, borderRadius: 12, backgroundColor: 'var(--color-accent)', color: '#fff', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px var(--color-accent-glow)' }}
          >
            {showIngest
